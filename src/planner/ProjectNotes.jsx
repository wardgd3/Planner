...
export default function ProjectNotes({ projectId }) {
  ...
  async function fetchAll() {
    ...
    const [{ data: t, error: e1 }, { data: n, error: e2 }] = await Promise.all([
      supabase.from('project_note_topics').select('*').eq('project_id', projectId).order('sort_order').order('created_at'),
      supabase.from('project_notes').select('*').eq('project_id', projectId).order('is_pinned', { ascending: false }).order('updated_at', { ascending: false }),
    ])
    if (e1 || e2) toast.error('Failed to load project notes')
    if (t) setTopics(t)
    if (n) setNotes(n)
    setLoading(false)
  }
  ...
  async function saveTopic() {
    ...
    setSaving(true)
    if (topicForm?.id) {
      const { error } = await supabase.from('project_note_topics').update({ name: topicName.trim(), color: topicColor, updated_at: new Date().toISOString() }).eq('id', topicForm.id)
      ...
      const { data, error } = await supabase.from('project_note_topics').insert({ name: topicName.trim(), color: topicColor, sort_order: maxOrder + 1, project_id: projectId }).select().single()
  ...
  async function deleteTopic(id) {
    const { error } = await supabase.from('project_note_topics').delete().eq('id', id)
  ...
  async function confirmDeleteNote() {
    ...
    const { error } = await supabase.from('project_notes').delete().eq('id', deleteConfirm.id)
  ...
  async function togglePin(note) {
    ...
    const { error } = await supabase.from('project_notes').update({ is_pinned: pinned, updated_at: new Date().toISOString() }).eq('id', note.id)
    if (error) toast.error('Failed to update note')
  ...
  if (loading) return <div className="pn-wrap"><p className="notes-loading">Loading notes…</p></div>
    ...
    <div className="pn-wrap">
      ...
      {topics.length === 0 && !topicForm ? (
        <div className="notes-empty">
          ...
          <p className="notes-empty-sub">Add a topic to start organizing notes for this project.</p>
        </div>
      ) : (
